...
class EdgeNetWithCategories(nn.Module):
    def __init__(self, input_dim=3, hidden_dim=8, edge_dim=2, output_dim=4, n_iters=1, aggr='add',
#                 norm=torch.tensor([1./500., 1./500., 1./54., 1/25., 1./1000.])):
        ...
        self.edgenetwork = nn.Sequential(nn.Linear(2*n_iters*hidden_dim + edge_dim, 2*hidden_dim),
                                         nn.ELU(),
                                         nn.Linear(2*hidden_dim, 2*hidden_dim),
                                         nn.ELU(),
    ...
    def forward(self, data):
        ...
        return self.edgenetwork(torch.cat([H_cat[row],H_cat[col],data.edge_attr.to(dtype=torch.float32)],dim=-1)).squeeze(-1)
